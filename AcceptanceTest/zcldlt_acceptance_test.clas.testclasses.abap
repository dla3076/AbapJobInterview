CLASS ltcl_unittest DEFINITION
  ...
  RISK LEVEL HARMLESS
  DURATION SHORT
  FRIENDS zcldlt_acceptance_test.

  PRIVATE SECTION.
    TYPES tt_integers TYPE STANDARD TABLE OF int4 WITH EMPTY KEY.

    CONSTANTS identifier          TYPE zdlttestresult-identifier VALUE 'ZCLDLT_ACCEPTANCE_TEST'.
...
CLASS ltcl_unittest IMPLEMENTATION.
  ...
  METHOD execute_test.
    " when + then
    DATA(test_instance) = zcldlt_acceptance_test=>create( identifier = identifier
                                                          method     = method ).
