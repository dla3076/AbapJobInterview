...
CLASS zcldlt_approval_test_testres DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC.

  PUBLIC SECTION.
    INTERFACES zifdlt_approval_test_testres.
...
CLASS zcldlt_approval_test_testres IMPLEMENTATION.
  METHOD constructor.
  ...
  METHOD zifdlt_approval_test_testres~save.
  ...
  METHOD zifdlt_approval_test_testres~add_result.
  ...
  METHOD zifdlt_approval_test_testres~select_latest_result.
    result-counter = calculate_counter( ).
    result-value   = zifdlt_approval_test_testres~select_result_by_counter( result-counter )-value.
  ENDMETHOD.

  METHOD zifdlt_approval_test_testres~select_result_by_counter.
    SELECT SINGLE method_parameter,
                  method_result
            FROM zdlttestresult
            WHERE identifier = @identifier
              AND method     = @method
