"! <p class="shorttext synchronized">Executes the approval tests</p>
CLASS zcldlt_approval_test DEFINITION
  PUBLIC
  FINAL
  CREATE PRIVATE.
  ...
  PUBLIC SECTION.
    ...
    TYPES tt_counters    TYPE STANDARD TABLE OF int4 WITH EMPTY KEY.

    CLASS-METHODS create
      IMPORTING !identifier            TYPE zdlttestresult-identifier
                !method                TYPE zdlttestresult-method
      RETURNING VALUE(acceptance_test) TYPE REF TO zcldlt_approval_test
      RAISING   zcxdlt_parameter_initial.

    CLASS-METHODS create_by_test_result
      IMPORTING test_result            TYPE REF TO zifdlt_approval_test_testres
      RETURNING VALUE(acceptance_test) TYPE REF TO zcldlt_approval_test
      RAISING   zcxdlt_parameter_initial.
    ...
    "! <p class="shorttext synchronized"></p>
    "! Executes the testmethod with parameters based on the counters.<br/>
    "! The method has to be eiter public or this class has to be a friend.<br/>
    "! The type of the returning_parameter has to be a string.
    "! @parameter test_instance       | <p class="shorttext synchronized"></p>
    "! @parameter method_name         | <p class="shorttext synchronized"></p>
    "! @parameter counters            | <p class="shorttext synchronized"></p>
    "! @parameter returning_parameter | <p class="shorttext synchronized"></p>
    "! @parameter acceptance_test     | <p class="shorttext synchronized"></p>
    "! @raising   cx_dynamic_check    | <p class="shorttext synchronized"></p>
    "! @raising   cx_static_check     | <p class="shorttext synchronized"></p>
    METHODS execute_by_counter
      IMPORTING test_instance          TYPE REF TO object
                method_name            TYPE clike
                counters               TYPE tt_counters
                returning_parameter    TYPE abap_parmbind-name
      RETURNING VALUE(acceptance_test) TYPE REF TO zcldlt_approval_test
      RAISING   cx_dynamic_check
                cx_static_check.
  ...
  PRIVATE SECTION.
    ...
    DATA test_result TYPE REF TO zifdlt_approval_test_testres.
    ...
    "! <p class="shorttext synchronized"></p>
    "! Convert a JSON string to parameters
    "! @parameter json_string | <p class="shorttext synchronized"></p>
    "! @parameter parameters  | <p class="shorttext synchronized"></p>
    METHODS convert_json_to_parameters
      IMPORTING json_string       TYPE string
      RETURNING VALUE(parameters) TYPE tt_parameter_strings.
...
CLASS zcldlt_approval_test IMPLEMENTATION.
  METHOD create.
  ...
  METHOD build_test_result.
    test_result = NEW zcldlt_approval_test_testres( identifier = identifier
                                                      method     = method ).
  ENDMETHOD.
  ...
  METHOD execute_test_method.
    ...
    DATA(exp) = test_result->select_latest_result( ).

    cl_abap_unit_assert=>assert_equals( exp  = exp-value
                                        act  = result
                                        msg  = |{ exp-counter }: { parameter }|
  ...
  METHOD execute_by_counter.
    DATA(object_description) = CAST cl_abap_objectdescr( cl_abap_typedescr=>describe_by_object_ref( test_instance ) ).

    acceptance_test = me.
    LOOP AT counters ASSIGNING FIELD-SYMBOL(<counter>).

      DATA(result) = ||.
      DATA(exp) = test_result->select_result_by_counter( <counter> ).
      DATA(parameters) = convert_json_to_parameters( exp-parameter ).
      DATA(current_parameters) = VALUE abap_parmbind_tab( ( kind  = cl_abap_objectdescr=>receiving
                                                            name  = returning_parameter
                                                            value = REF #( result ) ) ).
      ...
      LOOP AT parameters ASSIGNING FIELD-SYMBOL(<parameter>).
        ...
        INSERT VALUE #( name = <parameter>-name
                        kind = cl_abap_objectdescr=>exporting )
               INTO TABLE current_parameters
               ASSIGNING FIELD-SYMBOL(<current_param>)
               ELSE UNASSIGN.
        IF <current_param> IS ASSIGNED.

          object_description->get_method_parameter_type( EXPORTING
                                                           p_method_name    = method_name
                                                           p_parameter_name = <parameter>-name
                                                         RECEIVING
                                                           p_descr_ref      = DATA(data_descr)
                                                         EXCEPTIONS
                                                           OTHERS           = 0 ).
          CREATE DATA <current_param>-value TYPE HANDLE data_descr.
          IF data_descr IS INSTANCE OF cl_abap_enumdescr.

            DATA(class) = match( val  = data_descr->absolute_name
                                 pcre = `(?<=\\CLASS=)\w*(?=\\TYPE=)` ).

            ASSIGN (class)=>(<parameter>-value)
          ...
          ELSE.
            ...
            <current_param>-value->* = <parameter>-value.
      ...
      cl_abap_unit_assert=>assert_equals( exp  = exp-value
                                          act  = result
                                          msg  = |{ <counter> }: { convert_parameter_to_json( current_parameters  ) }|
                                          quit = if_abap_unit_constant=>quit-no ).

    ENDLOOP.
  ENDMETHOD.

  METHOD convert_json_to_parameters.
    CALL TRANSFORMATION id
         SOURCE JSON json_string
